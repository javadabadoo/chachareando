...
import doo.daba.java.persistence.persitenceMapping.UserObjectMapping;
...
public class UserDaoImpl extends JdbcDaoSupport implements UserDao {
	...
	public final int insert(final User element) {
		...
		return super.getJdbcTemplate().update(
			PropertiesContainer.get("sql.insert.user"),
			new Object[] {
				element.getName(),
				element.getLastName(),
	...
	public User select(int id) {
		...
		return super.getJdbcTemplate().queryForObject(
			PropertiesContainer.get("sql.select.user.forId"),
			new Object[] {
				id
			},
	...
	public User select(String userAlias) {
        ...
        try {
            usuario =  super.getJdbcTemplate().queryForObject(
                PropertiesContainer.get("sql.select.user.foralias"),
                new Object[] {
                        userAlias
                },
                new UserObjectMapping(true));
	...
	public List<String> selectUserRoles(int userId) {
		...
		return super.getJdbcTemplate().query(
				PropertiesContainer.get("sql.select.user.role"),
				new Object[]{
                        userId
				},
	...
	public int delete(User element) {
		...
		return super.getJdbcTemplate().update(
			PropertiesContainer.get("sql.delete.user"),
			new Object[] {
				element.getUserAlias()
			});
    ...
    public int linkUserProfilePicture(int userId, int imageId) {
        return super.getJdbcTemplate().update(PropertiesContainer.get("sql.insert.user.link.picture"), userId, imageId);
    }
}